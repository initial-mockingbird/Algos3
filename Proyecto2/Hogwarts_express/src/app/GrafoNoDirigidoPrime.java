...
public class GrafoNoDirigidoPrime {
     ...
     * @throws IOException
     */
    public static String result(GrafoNoDirigido g, Vertice inicio, Vertice fin, String archivo_lineas)
            throws IOException {
        HashSet<Arista> listaLados = g.aristas();
        ...
        HashSet<AristaPrime> nlistaLados = new HashSet<AristaPrime>();
        ...
        for (int j = 0; j < aux.length; j++) {
            for (int k = j; k < aux.length; k++) {
                if (aux[j].get_vertice().equals(aux[k].get_vertice())) {
                    toInsert = new AristaPrime(aux[j], aux[k], (double) 1);
                } else {
     ...
     */
    private static HashSet<VerticePrime> succ(HashSet<AristaPrime> arista, VerticePrime v) {
        Iterator<AristaPrime> i = arista.iterator();
        AristaPrime e;
     ...
     */
    private static String dijkstra(HashSet<VerticePrime> vertices, HashSet<AristaPrime> arista, VerticePrime inicio,
            VerticePrime fin) {
        ...
        while (queue.size() > 0) {
            ...
            while (j.hasNext()) {
                ...
                if (costo[vert2index.get(w)] > (costo[vert2index.get(succ)] + costo(w, succ))) {
                    costo[vert2index.get(w)] = costo[vert2index.get(succ)] + costo(w, succ);
                    p[vert2index.get(w)] = succ;
                }
            }
